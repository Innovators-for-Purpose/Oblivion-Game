...
var entered = false
var exited = false
onready var alex = get_node("/root/Main2/AlexStates")
onready var cam = get_node("/root/Main2/AlexStates/Camera2D")
...
func face_Alex():
	if alex.player.x > $Avery.global_position.x:
		$Avery.set_flip_h(true)
	if alex.player.x < $Theo.global_position.x:
