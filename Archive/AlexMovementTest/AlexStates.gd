...
func _input(event: InputEvent):
	...
	if event is InputEventMouseButton and event.pressed and can_grapple and not $GrappleLineDetect.is_colliding():
		...
		if hook_position.x + GRAPPLE_RADIUS >= mouse.x and hook_position.x - GRAPPLE_RADIUS <= mouse.x and hook_position.y + GRAPPLE_RADIUS >= mouse.y and hook_position.y - GRAPPLE_RADIUS <= mouse.y:
			print ('yay')
			$Chain.shoot(hook_position - self.position)
...
#			move_and_slide(velocity)
			self.position = lerp(self.position, hook_position, .2)
